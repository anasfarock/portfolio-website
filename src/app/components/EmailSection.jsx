  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setError(true); // Email sending failed
      }
    } catch (err) {
      ...
      <div>
        {emailSubmitted ? (
          ...
          <form className="flex flex-col" onSubmit={handleSubmit}>
            ...
            {loading ? (
              <button type="button" className="bg-primary-500 text-white font-medium py-2.5 px-5 rounded-lg w-full" disabled>
                Sending...
              </button>
            ) : (
            ...
            {error && <p className="text-red-500 text-sm mt-2">Failed to send email. Please try again.</p>}
          </form>
        )}
      </div>
    </section>
